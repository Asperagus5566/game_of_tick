...
back = (200, 255, 255)
main_window = pygame.display.set_mode((500, 500))#main_window
main_window.fill(back)
clock = pygame.time.Clock()
#класс прямоунольника
class Area():
...
while True:
    #Отрисовка карточек и отабражение кликов
    if wait == 0:
        wait = 20#Сколько тиков надпись будет на одном месте 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button ==1:
                ...
                for i in range(num_cards):
                    ...
                    if cards[i].collidepoint(x, y):
                        if i + 1 == click:
                            cards[i].color(GREEN) #если на карте еcть надпись перекоашиваем в зеленый , плюс очко
                            points += 1
                        
                    else: #Иначе перекрашиваем красным , минус очко
                        cards[i].color(RED)
                        points -= 1
                    cards[i].fill()
